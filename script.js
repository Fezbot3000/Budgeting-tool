...
function updateAccordion() {
    ...
    cycleDates.forEach((dates, index) => {
        ...
        accordionContainer.innerHTML += `<button class="accordion">${dates.start.toDateString()} - ${dates.end.toDateString()}<span class="leftover">Leftover: $${leftoverAmount.toFixed(2)}</span></button><div class="panel"><div class="pay-cycle"><table><tr><td colspan="2">Income:</td><td class="positive">$${income.toFixed(2)}</td></tr><tr><td colspan="2">Total Bills:</td><td class="negative">-$${cycleTotal.toFixed(2)}</td></tr>${cycleBills}</table></div></div>`;chartData.dates.push(dates.start.toDateString());
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    if (income) {
        ...
        const incomeTable = document.getElementById('incomeTable');
        incomeTable.innerHTML += `<tr><td>${payFrequency}</td><td>$${income.toFixed(2)}</td><td>${localStorage.getItem('payday')}</td><td>$${yearlyIncome.toFixed(2)}</td></tr>`;
        document.getElementById('step1').classList.add('hidden');
        document.getElementById('step2').classList.remove('hidden');
    }
    updateBillsTable();
